...
console.log('added item as', addedItem);
console.log('Supply changes is now', supplyChanges);
...
//  "in" loop : the variable is going to be the index of the array
for (let i in supplyChanges) {
  const num = supplyChanges[i];
...
// "of" loop means the variable will be the value of the item in
for (num of supplyChanges){
  if(num < 0){
    stretchNegatives.push(num); 
  } else if (num > 0) {
    stretchPositives.push(num); 
  } else (stretchZeroes.push(num));
